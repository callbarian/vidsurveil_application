...
class GroundTruthLoader(object):
    ...
    AIHUB = 'mini'
    AIHUB_LABEL_PATH = os.path.join(DATA_DIR,'mini/gt')
    ...
    def __call__(self, dataset):
        ...
        elif dataset == GroundTruthLoader.AIHUB:
            gt = self.__load_aihub_gt()
        else:
    ...
    @staticmethod
    def __load_aihub_gt():
        video_path_list = os.listdir(GroundTruthLoader.AIHUB_LABEL_PATH)

        video_path_list.sort()
        gt = []

        for video in video_path_list:
            gt.append(np.load(os.path.join(GroundTruthLoader.AIHUB_LABEL_PATH, video)))
...
def precision_recall_auc(loss_file):
    ...
    for sub_loss_file in loss_file_list:
        ...
        result_path = os.getcwd()+'/result_auc/'
        if not os.path.exists(result_path):
            os.makedirs(result_path)
        auprc ={}
        auprc['recall'] = recall
        auprc['precision'] = precision 
        scio.savemat(result_path+'future_pred_auprc.mat',{'auprc':auprc}) 
...
def compute_auc(loss_file):
    ...
    for sub_loss_file in loss_file_list:
        ...
        for i in range(num_videos):
            ...
            scores = np.concatenate((scores, distance[DECIDABLE_IDX:]), axis=0)
            labels = np.concatenate((labels, gt[i][DECIDABLE_IDX:]), axis=0)
            print('score: ',distance[DECIDABLE_IDX:])
            print('labels: ',gt[i][DECIDABLE_IDX:])
        fpr, tpr, thresholds = metrics.roc_curve(labels, scores, pos_label=0)
        ...
        auroc['fpr'] = fpr
        auroc['tpr'] = tpr
        scio.savemat(result_path+'future_pred_auroc.mat',{'auroc':auroc})

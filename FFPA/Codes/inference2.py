...
with tf.Session(config=config) as sess:
    ...
    def inference_func(ckpt, dataset_name, evaluate_name):
        ...
        #results = evaluate2.evaluate(evaluate_name, pickle_path)
        print(results)
    ...
    if os.path.isdir(snapshot_dir):
        ...
        while True:
            ...
            #evaluate2.evaluate('compute_auc', psnr_dir)
            time.sleep(60)
    else:
        inference_func(snapshot_dir, dataset_name, 'precision_recall_auc')
        #inference_func(snapshot_dir, dataset_name,'compute_auc')
